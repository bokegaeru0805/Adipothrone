    ...
    public class FlowchartWindow : EventWindow
    {
        ...
        protected class BlockCopy
        {
            ...
            internal Block PasteBlock(FlowchartWindow flowWind, Flowchart flowchart)
            {
                ...
                {
                    var newCommand =
                        Undo.AddComponent(flowchart.gameObject, command.type) as Command;
                ...
                {
                    var newEventHandler =
                        Undo.AddComponent(flowchart.gameObject, eventHandler.type) as EventHandler;
                ...
                newBlock.BlockName = flowchart.GetUniqueBlockKey(
                    block.FindProperty("blockName").stringValue + " (Copy)"
                );
        ...
        protected virtual void OnInspectorUpdate()
        {
            ...
            if (Selection.activeGameObject == null && flowchart.SelectedBlock != null)
            {
        ...
        public static Flowchart GetFlowchart()
        {
            ...
            if (fungusState.SelectedFlowchart == null)
            {
                ...
                variableListAdaptor == null
                || variableListAdaptor.TargetFlowchart != fungusState.SelectedFlowchart
            )
            {
                var fsSO = new SerializedObject(fungusState.SelectedFlowchart);
                variableListAdaptor = new VariableListAdaptor(
                    fsSO.FindProperty("variables"),
                    fungusState.SelectedFlowchart
                );
            }
        ...
        protected void UpdateFilteredBlocks()
        {
            if (filterStale)
            {
                ...
                {
                    Debug.LogWarning(
                        "Null block found in filteredBlocks. May be a symptom of an underlying issue"
                    );
                }
        ...
        private bool IsCommandContentMatch(Block block)
        {
            try
            {
                return block.CommandList.Any(command =>
                    command
        ...
        protected virtual void HandleEarlyEvents(Event e)
        {
            switch (e.type)
            {
                case EventType.MouseDown:
                    ...
                    if (
                        !searchRect.Contains(e.mousePosition)
        ...
        private void StartControlSelection()
        {
            ...
            for (int i = 0; i < mouseDownSelectionState.Count; i++)
            {
                ...
                {
                    Debug.LogWarning(
                        "Null block found in mouseDownSelectionState. May be a symptom of an underlying issue"
                    );
                }
            }
        }
            ...
            GUILayout.BeginHorizontal(EditorStyles.toolbar);
            {
                ...
                if (GUILayout.Button(addButtonContent, EditorStyles.toolbarButton))
                {
                    ...
                    Vector2 newNodePosition = new Vector2(
                        50 / flowchart.Zoom - flowchart.ScrollPos.x,
                ...
                var newString = EditorGUILayout.TextField(
                    searchString,
                ...
                if (e.type == EventType.Repaint)
                {
                    ...
                    popupRect.height = Mathf.Min(
                        Mathf.Max(1, filteredBlocks.Count) * 16,
                        position.height - 22
                    );
                }
        ...
        protected virtual void DrawVariablesBlock(Event e)
        {
            ...
            GUILayout.BeginHorizontal();
            {
                GUILayout.BeginVertical(GUILayout.Width(440));
                {
                    ...
                    flowchart.VariablesScrollPos = GUILayout.BeginScrollView(
                        flowchart.VariablesScrollPos
                    );
                    {
                        GUILayout.Space(8);
        ...
        protected virtual void DrawBlockPopup(Event e)
        {
            ...
            {
                popupScroll = EditorGUILayout.BeginScrollView(
                    popupScroll,
                    GUIStyle.none,
                ...
                case MouseButton.Left:
                    if (!e.alt)
                    {
                        if (hitBlock != null)
                        {
                            ...
                            startDragPosition =
                                e.mousePosition / flowchart.Zoom - flowchart.ScrollPos;
                ...
                case MouseButton.Left:
                    ...
                    if (dragBlock != null)
                    {
                        for (int i = 0; i < flowchart.SelectedBlocks.Count; ++i)
                        {
                            ...
                            var tempRect = block._NodeRect;
                            var distance =
                                e.mousePosition / flowchart.Zoom
                                - flowchart.ScrollPos
                                - startDragPosition;
                        ...
                        if (hitBlock != null)
                        {
                            ...
                            if (Application.isPlaying)
                            {
                                ...
                                menu.AddItem(
                                    new GUIContent("Stop"),
                                    false,
                                    () => StopThisBlock(hitBlock)
                                );
                                menu.AddItem(
                                    new GUIContent("Execute"),
                                    false,
                                    () => ExecuteThisBlock(hitBlock, false)
                                );
                                menu.AddItem(
                                    new GUIContent("Execute (Stop All First)"),
                                    false,
                                    () => ExecuteThisBlock(hitBlock, true)
                                );
                            }
                            else
                            {
        ...
        private Rect CalcFlowchartWindowViewRect()
        {
            return new Rect(
                0,
                0,
                this.position.width / flowchart.Zoom,
                this.position.height / flowchart.Zoom
            );
        }
        ...
        protected virtual void DrawRectConnection(Rect rectA, Rect rectB, bool highlight)
        {
            ...
            var direction = (
                GetPointOnCurve(pointA, controlA, pointB, controlB, 0.6f) - point
            ).normalized;
            ...
            var connectionRectA = new Rect(
                connectionPointA.x - 4f,
                connectionPointA.y - 4f,
                8f,
                8f
            );
            var connectionPointB = pointB + directionB * 4f;
            var connectionRectB = new Rect(
                connectionPointB.x - 4f,
                connectionPointB.y - 4f,
                8f,
                8f
            );
        ...
        private static Vector2 GetPointOnCurve(
            Vector2 s,
            Vector2 st,
            Vector2 e,
            Vector2 et,
        ...
        protected static void ShowBlockInspector(Flowchart flowchart)
        {
            if (blockInspector == null)
            {
                ...
                blockInspector =
                    ScriptableObject.CreateInstance<BlockInspector>() as BlockInspector;
        ...
        protected virtual void Copy()
        {
            ...
            foreach (var block in flowchart.SelectedBlocks.Union(mouseDownSelectionState))
            {
        ...
        protected static List<Block> blockGraphicsUniqueListWorkSpace = new List<Block>();
        protected static List<Block> blockGraphicsConnectedWorkSpace = new List<Block>();
        ...
        protected virtual BlockGraphics GetBlockGraphics(Block block)
        {
            ...
            // 以前の行: graphics.tint = block.UseCustomTint ? block.Tint : defaultTint;

            // 新しい色の決定ロジック
            if (block.UseCustomTint)
            {
                // 1. ユーザーが「Custom Tint」を有効にしている場合は、その色を最優先します。
                graphics.tint = block.Tint;
            }
            else
            {
                // 2. Custom Tintが無効な場合、新しく追加したBlockTypeに応じて色を決定します。
                switch (block.TypeOfBlock)
                {
                    case BlockType.Story:
                        // Storyタイプのブロックは青色に設定します。
                        graphics.tint = new Color(0.3f, 0.5f, 0.9f, 1f);
                        break;

                    case BlockType.NPC:
                        // 落ち着いた緑色に設定
                        graphics.tint = new Color(0.4f, 0.7f, 0.4f, 1f);
                        break;

                    case BlockType.Default:
                    default:
                        // 3. Defaultタイプの場合は、Fungusの元のルール（イベント、分岐、通常）で決まった色を使用します。
                        graphics.tint = defaultTint;
                        break;
                }
            }
        ...
        private void DrawBlock(Block block, Rect scriptViewRect)
        {
            ...
            if (scriptViewRect.Overlaps(windowRelativeRect))
            {
                ...
                var brightness =
                    graphics.tint.r * 0.3 + graphics.tint.g * 0.59 + graphics.tint.b * 0.11;
                ...
                if (block.Description.Length > 0)
                {
                    ...
                    windowRelativeRect.height = descriptionStyle.CalcHeight(
                        content,
                ...
                if (block._EventHandler != null)
                {
                    ...
                    rect.height = handlerStyle.CalcHeight(
                        new GUIContent(handlerLabel),
                        block._NodeRect.width
                    );

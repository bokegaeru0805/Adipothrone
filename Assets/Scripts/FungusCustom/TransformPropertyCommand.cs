...
[CommandInfo("Transform", "Set Transform Property", "GameObjectのPosition, Rotation, Scaleを変更します。Tweenアニメーションにも対応しています。")]
    [AddComponentMenu("")]
    public class TransformPropertyCommand : Command
    {
        ...
        [BoxGroup("Target Settings")]
        [SerializeField] protected TransformMode transformMode = TransformMode.Position;

        // ScaleのときはWorld座標の概念が特殊なのでLocal固定に見せるなどの制御
        [BoxGroup("Target Settings")]
        [HideIf("IsScaleMode")] 
        [SerializeField] protected SpaceMode spaceMode = SpaceMode.World;

        [BoxGroup("Target Settings")]
        [SerializeField] protected OperationMode operationMode = OperationMode.Set;

        // --- 値 ---
        [BoxGroup("Value Settings")]
        [Tooltip("目標値、または加算する値")]
        [SerializeField] protected Vector3Data targetValue;
        ...
        private void ApplyValue(Vector3 value)
        {
            ...
            {
                case TransformMode.Position:
                    if (spaceMode == SpaceMode.World) t.position = value;
                    else t.localPosition = value;
                    break;
                case TransformMode.Rotation:
                    if (spaceMode == SpaceMode.World) t.eulerAngles = value;
                    else t.localEulerAngles = value;
                    break;
                case TransformMode.Scale:
                    t.localScale = value;
                    break;
        ...
        {
            if (targetTransform.Value == null) return "Error: No Target Transform";
